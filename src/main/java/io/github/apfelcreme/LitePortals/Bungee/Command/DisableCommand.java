    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (player.hasPermission("LitePortals.mod")) {
            if (args.length > 1) {
                ...
                LitePortals.getInstance().getDatabaseController().disableWorld(args[1], currentServer);
                ...
                System.out.println(disabledWorlds.toString());
                ...
                LitePortals.sendMessage(player.getUniqueId(), LitePortalsConfig.getInstance()
                        .getText("info.disable.disabled").replace("{0}", args[1]));
            } else {
                LitePortals.sendMessage(player.getUniqueId(), LitePortalsConfig.getInstance()
                        .getText("error.wrongUsage").replace("{0}", "/portal disable <Welt>"));
            }
        } else {
            LitePortals.sendMessage(player.getUniqueId(), LitePortalsConfig.getInstance()
