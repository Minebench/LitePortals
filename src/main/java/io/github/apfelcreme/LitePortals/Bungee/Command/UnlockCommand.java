    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (player.hasPermission("LitePortals.user")) {
            if (args.length > 1) {
                ...
                if (portal != null) {
                    LitePortals.getInstance().getDatabaseController().unlock(portal);
                    LitePortals.sendMessage(player.getUniqueId(), LitePortalsConfig.getInstance().getText("info.unlock.unlocked"));
                } else {
